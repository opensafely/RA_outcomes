...
forvalues i=2019/2021 {
    ...
    * Create table of number of appointment and mode where mode info available for all appts
    ...
    export delimited using ./output/op_long/bsr_op_appt_medium_`i'.csv
    restore
    tempfile tempfile
    preserve 
    ...
    export delimited using ./output/op_long/bsr_op_medium_chars_`i'.csv
    restore
    preserve 
    ...
    table1_mc, vars(age_cat cate \ male cate \ urban_rural_bin cate) clear
    append using tempfile
    export delimited using ./output/op_long/bsr_op_remote_chars_`i'.csv
    restore
    preserve
    keep if all_mode_available==0
    table1_mc, vars(age_cat cate \ male cate \ urban_rural_bin cate) clear
    append using tempfile
    export delimited using ./output/op_long/bsr_op_no_medium_chars_`i'.csv
    restore
    preserve
    keep if all_mode_available==2
    table1_mc, vars(age_cat cate \ male cate \ urban_rural_bin cate) clear
    append using tempfile
    export delimited using ./output/op_long/bsr_op_uk_medium_chars_`i'.csv
    restore
    }
